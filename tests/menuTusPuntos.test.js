...
function test_mapping_correcto() {
  const esperado = [
    ['predictions', 'PRO', 'Pronósticos', 'ball-pronosticos'],
    ['squad', 'PLA', 'Plantilla', 'ball-plantilla'],
    ['classification', 'CLA', 'Clasificación', 'ball-clasificacion'],
    ['eliminatorias', 'ELI', 'Eliminatorias', 'ball-eliminatorias'],
  ];
  assert.strictEqual(POINTS_MENU_TABS.length, 4, '4 pestañas');
  POINTS_MENU_TABS.forEach((t, i) => {
    const [tab, short, label, cls] = esperado[i];
    assert.strictEqual(t.tab, tab, `tab ${i}`);
    assert.strictEqual(t.short, short, `short ${i}`);
...
function test_html_bolitas_con_data_tab() {
  const html = buildPointsMenuHtml(0);
  for (const t of POINTS_MENU_TABS) {
    assert.ok(html.includes(`data-tab="${t.tab}"`), `data-tab ${t.tab}`);
    assert.ok(html.includes(`<span class="points-ball-label">${t.short}</span>`), `short ${t.short}`);
...
const tests = [
  ...
  test_html_bolitas_con_data_tab,
  test_html_chevron,
  test_suma_completa,
